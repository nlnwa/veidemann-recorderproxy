...
// be MITM'ed an error is returned.
func (ic *Interceptor) MITM(downstream net.Conn, upstream net.Conn) (newDown net.Conn, newUp net.Conn,
	err error) {
	rc := reconn.Wrap(downstream, maxTLSRecordSize)
	tlsDown := tls.Server(rc, ic.serverTLSConfig)
	...
	return tlsDown, tlsUp, tlsUp.Handshake()
}
...
func (ic *Interceptor) initCrypto() (err error) {
	...
	if ic.opts.Organization == "" {
		ic.opts.Organization = "Veidemann"
	}
